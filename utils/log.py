...
def save_autoencoder(state, is_best=True, file_name='autoencoder.pth.tar', opt=None):
    file_path = os.path.join(opt.ckpt, file_name)
    torch.save(state, file_path)


def save_ckpt2(state, is_best=True, file_name=['ckpt_best_lower.pth.tar', 'ckpt_last_lower.pth.tar'], opt=None):
    file_path = os.path.join(opt.ckpt, file_name[1])
    torch.save(state, file_path)
    if is_best:
        file_path = os.path.join(opt.ckpt, file_name[0])
