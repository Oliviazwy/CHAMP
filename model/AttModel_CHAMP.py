...
class AttModel(Module):
    ...
    def __init__(self, in_features=48, kernel_size=5, d_model=512, num_stage=2, dct_n=10, model_prob=None):
        ...
        self.gcn = GCN_CHAMP.GCN(input_feature=(dct_n) * 2, hidden_feature=d_model, p_dropout=0.3,
                           num_stage=num_stage,
                           node_n=in_features,model_prob=model_prob)

    def forward(self, src, output_n=10, input_n=50, itera=1, data_source=None):
        """
        ...
        for i in range(itera):
            ...
            att_tmp = score_tmp / (torch.sum(score_tmp, dim=2, keepdim=True))  # 32,1,1
            dct_att_tmp = torch.matmul(att_tmp, src_value_tmp)[:, 0].reshape(
                [bs, -1, dct_n])  # 32,75,20

            input_gcn = src_tmp[:, idx]
            dct_in_tmp = torch.matmul(dct_m[:dct_n].unsqueeze(dim=0), input_gcn).transpose(1, 2)  # 32,75,20
            dct_in_tmp = torch.cat([dct_in_tmp, dct_att_tmp], dim=-1)
            dct_out_tmp = self.gcn(dct_in_tmp,data_source=data_source)
